...
<script type="text/javascript">
	...
	$("#search").on("click", function() {
		var status = $("#status").combobox("getValue");
	 	var title = $("#title").val();
		$("#list").datagrid({
			url:'${z:u("/news/image_list")}?title='+title+"&status="+status
		});
	});
	
	$("#add").click(function(){
		App.popup('${z:u('news/image_add')}',{
			title:"增加资讯",
			width: 800,
			height: 600
		});
	...
	if(row==null){
		$.messager.alert("提示","请先选择一条记录","warning");
 			...
 			App.popup('${z:u('news/image_edit')}?id='+row.id, {
 				title:"资讯编辑",
				width: 800,
	     		height: 600
			});
	...
	$("#delete").click(function(){
		...
		if(row == null){
			...
			App.ajax('${z:u("/news/image_delete")}?id='+row.id,{
	            msg: "确定要删除此条资讯吗？",
	            type: "POST"
	        });
		}
