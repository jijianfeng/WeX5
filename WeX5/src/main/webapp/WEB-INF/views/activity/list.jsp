	...
	<div data-options="region:'center',border:false">
		<table class="jq-datagrid" fit="true" id="list" data-options="{
			...
			columns: [[
				...
				{field:'joinNumber',title:'已参加人数',width:20,sortable:'true'},
				{field:'allNumber',title:'满载',width:20,sortable:'true'},
				{field:'addUser',title:'发布人',width:20,sortable:'true'},
				{field:'addTime',title:'发布时间',width:20,sortable:'true'},
				{field:'endTime',title:'报名截止时间',width:20,sortable:'true'},
...
<script type="text/javascript">
	...
	$("#search").on("click", function() {
		var status = $("#status").combobox("getValue");
	 	var title = $("#title").val();
		$("#list").datagrid({
			url:'${z:u("activity/list")}?title='+title+"&status="+status
		});
	});
	
	$("#add").click(function(){
		App.popup('${z:u('activity/list_add')}',{
			title:"增加资讯",
			width: 800,
			height: 600
		});
	...
	if(row==null){
		$.messager.alert("提示","请先选择一条记录","warning");
 			...
 			App.popup('${z:u('activity/list_edit')}?id='+row.id, {
 				title:"资讯编辑",
			width: 800,
     		height: 600
			});
	...
	$("#delete").click(function(){
		...
		if(row == null){
			...
			App.ajax('${z:u("activity/list_delete")}?id='+row.id,{
	            msg: "确定要删除此条资讯吗？",
	            type: "POST"
	            });
		}
	...
	function imgShow(v,r) {
		if (v == '') {
	        ...
	        return '<a class="J_imgShow" href='+v+'><img src='+v+' style="height: 100px;" /></a>';
	    }
	}
